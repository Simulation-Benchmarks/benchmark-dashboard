...
export async function loadAnalysis(runs: Run[]): Promise<RunAnalysisData> {
  ...
  const numeric = new Set(columns.filter((column) => {
    const values = rawRows.map((row) => row[column.key]).filter((value) => value !== null && value !== undefined && value !== '');
  ...
  return {
    ...
    payload: { benchmark: benchmarkLabel, software_name: runs.length === 1 ? runs[0].software_name : null, run_count: runs.length, columns, rows },
  };
}
